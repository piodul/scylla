...
private:
    ...
    inheriting_concrete_execution_stage<
            ...
            storage_proxy*,
            std::vector<mutation>,
            lw_shared_ptr<coordinator_mutate_options>,
...
private:
    ...
    result<response_id_type> create_write_response_handler_helper(schema_ptr s, const dht::token& token,
            std::unique_ptr<mutation_holder> mh, db::write_type type, lw_shared_ptr<coordinator_mutate_options> coordinator_options);
    result<response_id_type> create_write_response_handler(replica::keyspace& ks, db::write_type type, std::unique_ptr<mutation_holder> m, lw_shared_ptr<coordinator_mutate_options> coordinator_options, inet_address_vector_replica_set targets,
    ...
    result<response_id_type> create_write_response_handler(const mutation&, db::write_type type, lw_shared_ptr<coordinator_mutate_options> coordinator_options);
    result<response_id_type> create_write_response_handler(const hint_wrapper&, db::write_type type, lw_shared_ptr<coordinator_mutate_options> coordinator_options);
    result<response_id_type> create_write_response_handler(const std::unordered_map<gms::inet_address, std::optional<mutation>>&, db::write_type type, lw_shared_ptr<coordinator_mutate_options> coordinator_options);
    result<response_id_type> create_write_response_handler(const std::tuple<lw_shared_ptr<paxos::proposal>, schema_ptr, shared_ptr<paxos_response_handler>, dht::token>& proposal, db::write_type type, lw_shared_ptr<coordinator_mutate_options> coordinator_options);
    result<response_id_type> create_write_response_handler(const std::tuple<lw_shared_ptr<paxos::proposal>, schema_ptr, dht::token, inet_address_vector_replica_set>& meta, db::write_type type, lw_shared_ptr<coordinator_mutate_options> coordinator_options);
    ...
    future<result<unique_response_handler_vector>> mutate_prepare(Range&& mutations, db::write_type type, lw_shared_ptr<coordinator_mutate_options> coordinator_options, CreateWriteHandler handler);
    template<typename Range>
    future<result<unique_response_handler_vector>> mutate_prepare(Range&& mutations, db::write_type type, lw_shared_ptr<coordinator_mutate_options> coordinator_options);
    ...
    template<typename Range>
    future<result<>> mutate_internal(Range mutations, bool counter_write, lw_shared_ptr<coordinator_mutate_options> coordinator_options, std::optional<clock_type::time_point> timeout_opt = { }, lw_shared_ptr<cdc::operation_result_tracker> cdc_tracker = { });
    ...
    future<> mutate_counters_on_leader(std::vector<frozen_mutation_and_schema> mutations, lw_shared_ptr<coordinator_mutate_options> coordinator_options, clock_type::time_point timeout);
    future<> mutate_counter_on_leader_and_replicate(const schema_ptr& s, frozen_mutation m, lw_shared_ptr<coordinator_mutate_options> coordinator_options, clock_type::time_point timeout);

    gms::inet_address find_leader_for_counter_update(const mutation& m, db::consistency_level cl);

    future<result<>> do_mutate(std::vector<mutation> mutations, lw_shared_ptr<coordinator_mutate_options> coordinator_options, clock_type::time_point timeout, bool, lw_shared_ptr<cdc::operation_result_tracker> cdc_tracker);
    ...
    template<typename Range>
    future<> mutate_counters(Range&& mutations, lw_shared_ptr<coordinator_mutate_options> coordinator_options, clock_type::time_point timeout);
...
private:
    ...
    future<rpc::tuple<Elements..., std::optional<replica::exception_variant>>> encode_replica_exception_for_rpc(future<rpc::tuple<Elements...>>&& f, auto&& default_tuple_maker);
    ...
    future<rpc::tuple<foreign_ptr<lw_shared_ptr<query::result>>, cache_temperature, std::optional<replica::exception_variant>>> handle_read_data(const rpc::client_info& cinfo, rpc::opt_time_point t, query::read_command cmd, ::compat::wrapping_partition_range pr, rpc::optional<query::digest_algorithm> oda);
    future<rpc::tuple<foreign_ptr<lw_shared_ptr<reconcilable_result>>, cache_temperature, std::optional<replica::exception_variant>>> handle_read_mutation_data(const rpc::client_info& cinfo, rpc::opt_time_point t, query::read_command cmd, ::compat::wrapping_partition_range pr);
    future<rpc::tuple<query::result_digest, long, cache_temperature, std::optional<replica::exception_variant>>> handle_read_digest(const rpc::client_info& cinfo, rpc::opt_time_point t, query::read_command cmd, ::compat::wrapping_partition_range pr, rpc::optional<query::digest_algorithm> oda);
...
public:
    ...
    */
    future<> mutate(std::vector<mutation> mutations, lw_shared_ptr<coordinator_mutate_options> coordinator_options, clock_type::time_point timeout, bool raw_counters = false);
    ...
    */
    future<result<>> mutate_result(std::vector<mutation> mutations, lw_shared_ptr<coordinator_mutate_options> coordinator_options, clock_type::time_point timeout, bool raw_counters = false);

    paxos_participants
    get_paxos_participants(const sstring& ks_name, const dht::token& token, db::consistency_level consistency_for_paxos);

    future<> replicate_counter_from_leader(mutation m, lw_shared_ptr<coordinator_mutate_options> coordinator_options, clock_type::time_point timeout);

    future<result<>> mutate_with_triggers(std::vector<mutation> mutations, lw_shared_ptr<coordinator_mutate_options> coordinator_options,
    ...
    * @param tr_state trace state handle
    */
    future<> mutate_atomically(std::vector<mutation> mutations, lw_shared_ptr<coordinator_mutate_options> coordinator_options, clock_type::time_point timeout);
    ...
    * of the exception on the exception handling path.
    */
    future<result<>> mutate_atomically_result(std::vector<mutation> mutations, lw_shared_ptr<coordinator_mutate_options> coordinator_options, clock_type::time_point timeout);
