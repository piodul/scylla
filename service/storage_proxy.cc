...
static thread_local const utils::updateable_value_source<db::hints::host_filter> allow_all_hint_filter{db::hints::host_filter(db::hints::host_filter::enabled_for_all_tag())};
...
storage_proxy::storage_proxy(distributed<database>& db, storage_proxy::config cfg, db::view::node_update_backlog& max_view_update_backlog,
    ...
    , _hints_manager(_db.local().get_config().hints_directory(), _db.local().get_config().hinted_handoff_enabled, _db.local().get_config().max_hint_window_in_ms(), _hints_resource_manager, _db)
    , _hints_for_views_manager(_db.local().get_config().view_hints_directory(), utils::updateable_value<db::hints::host_filter>(allow_all_hint_filter), _db.local().get_config().max_hint_window_in_ms(), _hints_resource_manager, _db)
    ...
    slogger.trace("hinted DCs: {}", _db.local().get_config().hinted_handoff_enabled());
