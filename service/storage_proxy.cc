...
bo::failure_type<exceptions::coordinator_exception_container> coordinator_failure(Ex&& ex) {
    return bo::failure(exceptions::coordinator_exception_container(std::move(ex)));
}

template<typename T = void>
future<result<T>> then_ok_result(future<T>&& f) {
    if constexpr (std::is_void_v<T>) {
        ...
        return f.then([] (T&& t) {
            return make_ready_future<result<T>>(bo::success(std::move(t)));
        });
    }
}
    ...
    virtual ~abstract_write_response_handler() {
        ...
        if (_cl_achieved) {
            ...
            if (_error == error::TIMEOUT) {
                _ready.set_value(coordinator_failure(mutation_write_timeout_exception(get_schema()->ks_name(), get_schema()->cf_name(), _cl, _cl_acks, _total_block_for, _type)));
...
future<result<>> storage_proxy::mutate_begin(unique_response_handler_vector ids, db::consistency_level cl,
    ...
    return map_reduce(ids, [this, cl, timeout_opt] (unique_response_handler& protected_response) {
        ...
        return f;
    }, result<>(bo::success()), utils::result_reducer{});
}
    ...
    public:
        ...
        future<> async_remove_from_batchlog() {
            ...
            return send_batchlog_mutation(std::move(m), db::consistency_level::ANY).then_wrapped([] (future<result<>> f) {
                ...
                } else if (result<> res = f.get(); res.has_error()) {
                    res.assume_error().accept(print_exception);
                }
            });
        };
        ...
        future<result<>> run() {
            return _p.mutate_prepare(_mutations, _cl, db::write_type::BATCH, _trace_state, _permit).then([this] (unique_response_handler_vector ids) {
                return sync_write_to_batchlog().then([this, ids = std::move(ids)] (result<> res) mutable {
                    if (res.has_error()) {
                        return make_ready_future<result<>>(std::move(res));
                    }
                    ...
                    return then_ok_result(async_remove_from_batchlog());
                });
            });
        }
    };
