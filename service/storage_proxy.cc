...
public:
    ...
    virtual ~abstract_write_response_handler() {
        ...
        if (_cl_achieved) {
            ...
            // TODO: What should we do on std::monostate?
...
void storage_proxy::send_to_live_endpoints(storage_proxy::response_id_type response_id, clock_type::time_point timeout)
{
    ...
    for (typename decltype(dc_groups)::value_type& dc_targets : all) {
        ...
        (void)f.handle_exception([response_id, forward_size, coordinator, handler_ptr, p = shared_from_this(), &stats] (std::exception_ptr eptr) {
            ...
            replica::exception_variant ex = replica::encode_replica_exception(std::move(eptr));
            // TODO: Some exceptions printed here were originally silenced
            if (auto* u = std::get_if<replica::unknown_exception>(&ex.reason)) {
                slogger.error("exception during mutation write to {}: {}", coordinator, u->what());
            }
...
void storage_proxy::init_messaging_service(shared_ptr<migration_manager> mm) {
    ...
    ser::storage_proxy_rpc_verbs::register_read_data(&ms, [mm] (const rpc::client_info& cinfo, rpc::opt_time_point t, query::read_command cmd, ::compat::wrapping_partition_range pr, rpc::optional<query::digest_algorithm> oda) {
        ...
        return do_with(std::move(pr), std::move(sp), std::move(trace_state_ptr), [&cinfo, cmd = make_lw_shared<query::read_command>(std::move(cmd)), src_addr = std::move(src_addr), da, t, mm] (::compat::wrapping_partition_range& pr, shared_ptr<storage_proxy>& p, tracing::trace_state_ptr& trace_state_ptr) mutable {
            ...
            return mm->get_schema_for_read(cmd->schema_version, std::move(src_addr), p->_messaging).then([cmd, da, &pr, &p, &trace_state_ptr, t] (schema_ptr s) {
            ...
            }).then_wrapped([&trace_state_ptr, src_ip] (future<rpc::tuple<foreign_ptr<lw_shared_ptr<query::result>>, cache_temperature>> f) mutable {
                ...
                if (!f.failed()) {
                    ...
                    return final_tuple_type(std::move(result), std::move(temp), std::nullopt);
                } else {
                    auto ex = replica::encode_replica_exception(f.get_exception());
                    return final_tuple_type(foreign_ptr<lw_shared_ptr<query::result>>(), cache_temperature::invalid(), std::make_optional(std::move(ex)));
                }
            });
        });
    });
    ser::storage_proxy_rpc_verbs::register_read_mutation_data(&ms, [mm] (const rpc::client_info& cinfo, rpc::opt_time_point t, query::read_command cmd, ::compat::wrapping_partition_range pr) {
        ...
        return do_with(std::move(pr),
            ...
            return mm->get_schema_for_read(cmd->schema_version, std::move(src_addr), p->_messaging).then([cmd, &pr, &p, &trace_state_ptr, &unwrapped, t] (schema_ptr s) mutable {
            ...
            }).then_wrapped([&trace_state_ptr, src_ip] (future<rpc::tuple<foreign_ptr<lw_shared_ptr<reconcilable_result>>, cache_temperature>> f) mutable {
                ...
                if (!f.failed()) {
                    ...
                    auto ex = replica::encode_replica_exception(f.get_exception());
                    return final_tuple_type(foreign_ptr<lw_shared_ptr<reconcilable_result>>(), cache_temperature::invalid(), std::make_optional(std::move(ex)));
                }
            });
        });
    });
    ser::storage_proxy_rpc_verbs::register_read_digest(&ms, [mm] (const rpc::client_info& cinfo, rpc::opt_time_point t, query::read_command cmd, ::compat::wrapping_partition_range pr, rpc::optional<query::digest_algorithm> oda) {
        ...
        return do_with(std::move(pr), get_local_shared_storage_proxy(), std::move(trace_state_ptr), [&cinfo, cmd = make_lw_shared<query::read_command>(std::move(cmd)), src_addr = std::move(src_addr), da, t, mm] (::compat::wrapping_partition_range& pr, shared_ptr<storage_proxy>& p, tracing::trace_state_ptr& trace_state_ptr) mutable {
            ...
            return mm->get_schema_for_read(cmd->schema_version, std::move(src_addr), p->_messaging).then([cmd, &pr, &p, &trace_state_ptr, t, da] (schema_ptr s) {
            ...
            }).then_wrapped([&trace_state_ptr, src_ip] (future<rpc::tuple<query::result_digest, api::timestamp_type, cache_temperature>> f) mutable {
                ...
                if (!f.failed()) {
                    ...
                    return final_tuple_type(std::move(result), timestamp, std::move(temp), std::nullopt);
                } else {
                    auto ex = replica::encode_replica_exception(f.get_exception());
                    return final_tuple_type(query::result_digest(), api::missing_timestamp, cache_temperature::invalid(), std::make_optional(std::move(ex)));
                }
            });
        });
    });
