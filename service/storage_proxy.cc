...
public:
    abstract_write_response_handler(shared_ptr<storage_proxy> p, replica::keyspace& ks, db::consistency_level cl, db::write_type type,
            ...
            storage_proxy::write_stats& stats, service_permit permit, db::allow_per_partition_rate_limit rate_limit, size_t pending_endpoints = 0,
            ...
            : _id(p->get_next_response_id()), _proxy(std::move(p)), _trace_state(trace_state), _cl(cl), _type(type), _mutation_holder(std::move(mh)), _targets(std::move(targets)),
              _dead_endpoints(std::move(dead_endpoints)), _allow_per_partition_rate_limit(rate_limit), _stats(stats), _expire_timer([this] { timeout_cb(); }), _permit(std::move(permit)) {
...
public:
    datacenter_write_response_handler(shared_ptr<storage_proxy> p, replica::keyspace& ks, db::consistency_level cl, db::write_type type,
            ...
            storage_proxy::write_stats& stats, service_permit permit, db::allow_per_partition_rate_limit allow_limit) :
                abstract_write_response_handler(std::move(p), ks, cl, type, std::move(mh),
                        std::move(targets), std::move(tr_state), stats, std::move(permit), allow_limit, db::count_local_endpoints(pending_endpoints), std::move(dead_endpoints)) {
...
public:
    write_response_handler(shared_ptr<storage_proxy> p, replica::keyspace& ks, db::consistency_level cl, db::write_type type,
            ...
            storage_proxy::write_stats& stats, service_permit permit, db::allow_per_partition_rate_limit allow_limit) :
                abstract_write_response_handler(std::move(p), ks, cl, type, std::move(mh),
                        std::move(targets), std::move(tr_state), stats, std::move(permit), allow_limit, pending_endpoints.size(), std::move(dead_endpoints)) {
    ...
    view_update_write_response_handler(shared_ptr<storage_proxy> p, replica::keyspace& ks, db::consistency_level cl,
            ...
            storage_proxy::write_stats& stats, service_permit permit, db::allow_per_partition_rate_limit allow_limit):
                write_response_handler(p, ks, cl, db::write_type::VIEW, std::move(mh),
                        std::move(targets), pending_endpoints, std::move(dead_endpoints), std::move(tr_state), stats, std::move(permit), allow_limit) {
...
public:
    datacenter_sync_write_response_handler(shared_ptr<storage_proxy> p, replica::keyspace& ks, db::consistency_level cl, db::write_type type,
        ...
        abstract_write_response_handler(std::move(p), ks, cl, type, std::move(mh), targets, std::move(tr_state), stats, std::move(permit), allow_limit, 0, dead_endpoints) {
        auto& snitch_ptr = locator::i_endpoint_snitch::get_local_snitch_ptr();
                             ...
                             storage_proxy::write_stats& stats, service_permit permit, db::allow_per_partition_rate_limit allow_limit)
{
    ...
    if (db::is_datacenter_local(cl)) {
        h = ::make_shared<datacenter_write_response_handler>(shared_from_this(), ks, cl, type, std::move(m), std::move(targets), pending_endpoints, std::move(dead_endpoints), std::move(tr_state), stats, std::move(permit), allow_limit);
    } else if (cl == db::consistency_level::EACH_QUORUM && rs.get_type() == locator::replication_strategy_type::network_topology){
        h = ::make_shared<datacenter_sync_write_response_handler>(shared_from_this(), ks, cl, type, std::move(m), std::move(targets), pending_endpoints, std::move(dead_endpoints), std::move(tr_state), stats, std::move(permit), allow_limit);
    } else if (type == db::write_type::VIEW) {
        h = ::make_shared<view_update_write_response_handler>(shared_from_this(), ks, cl, std::move(m), std::move(targets), pending_endpoints, std::move(dead_endpoints), std::move(tr_state), stats, std::move(permit), allow_limit);
    } else {
        h = ::make_shared<write_response_handler>(shared_from_this(), ks, cl, type, std::move(m), std::move(targets), pending_endpoints, std::move(dead_endpoints), std::move(tr_state), stats, std::move(permit), allow_limit);
    }
...
storage_proxy::create_write_response_handler_helper(schema_ptr s, const dht::token& token, std::unique_ptr<mutation_holder> mh,
    ...
    return create_write_response_handler(ks, cl, type, std::move(mh), std::move(live_endpoints), pending_endpoints,
            std::move(dead_endpoints), std::move(tr_state), get_stats(), std::move(permit), allow_limit);
}
...
storage_proxy::create_write_response_handler(const std::unordered_map<gms::inet_address, std::optional<mutation>>& m, db::consistency_level cl, db::write_type type, tracing::trace_state_ptr tr_state, service_permit permit, db::allow_per_partition_rate_limit allow_limit) {
    ...
    return create_write_response_handler(ks, cl, type, std::move(mh), std::move(endpoints), inet_address_vector_topology_change(), inet_address_vector_topology_change(), std::move(tr_state), get_stats(), std::move(permit), allow_limit);
}
...
storage_proxy::create_write_response_handler(const std::tuple<lw_shared_ptr<paxos::proposal>, schema_ptr, dht::token, inet_address_vector_replica_set>& meta,
    ...
    return create_write_response_handler(ks, cl, db::write_type::CAS, std::make_unique<cas_mutation>(std::move(commit), s, nullptr), std::move(endpoints),
                    inet_address_vector_topology_change(), inet_address_vector_topology_change(), std::move(tr_state), get_stats(), std::move(permit), allow_limit);
}
...
future<storage_proxy::unique_response_handler_vector> storage_proxy::mutate_prepare(Range&& mutations, db::consistency_level cl, db::write_type type, tracing::trace_state_ptr tr_state, service_permit permit, db::allow_per_partition_rate_limit allow_limit) {
...
storage_proxy::mutate_internal(Range mutations, db::consistency_level cl, bool counters, tracing::trace_state_ptr tr_state, service_permit permit,
    ...
    return mutate_prepare(mutations, cl, type, tr_state, std::move(permit), allow_limit).then([this, cl, timeout_opt, tracker = std::move(cdc_tracker),
    ...
    public:
        ...
        future<result<>> send_batchlog_mutation(mutation m, db::consistency_level cl = db::consistency_level::ONE) {
            return _p.mutate_prepare<>(std::array<mutation, 1>{std::move(m)}, cl, db::write_type::BATCH_LOG, _permit, [this] (const mutation& m, db::consistency_level cl, db::write_type type, service_permit permit) {
                ...
                // TODO: Should we rate limit here or not?
                return _p.create_write_response_handler(ks, cl, type, std::make_unique<shared_mutation>(m), _batchlog_endpoints, {}, {}, _trace_state, _stats, std::move(permit), db::allow_per_partition_rate_limit::no);
        ...
        future<result<>> run() {
            return _p.mutate_prepare(_mutations, _cl, db::write_type::BATCH, _trace_state, _permit, db::allow_per_partition_rate_limit::no).then([this] (unique_response_handler_vector ids) {
...
storage_proxy::query_result_local_digest(schema_ptr s, lw_shared_ptr<query::read_command> cmd, const dht::partition_range& pr, tracing::trace_state_ptr trace_state, storage_proxy::clock_type::time_point timeout, query::digest_algorithm da, db::allow_per_partition_rate_limit allow_limit) {
    return query_result_local(std::move(s), std::move(cmd), pr, query::result_options::only_digest(da), std::move(trace_state), timeout, allow_limit).then([] (rpc::tuple<foreign_ptr<lw_shared_ptr<query::result>>, cache_temperature> result_and_hit_rate) {
...
storage_proxy::query_result_local(schema_ptr s, lw_shared_ptr<query::read_command> cmd, const dht::partition_range& pr, query::result_options opts,
                                  tracing::trace_state_ptr trace_state, storage_proxy::clock_type::time_point timeout, db::allow_per_partition_rate_limit allow_limit) {
    ...
    if (pr.is_singular()) {
        ...
        get_stats().replica_cross_shard_ops += shard != this_shard_id();
        return _db.invoke_on(shard, _read_smp_service_group, [gs = global_schema_ptr(s), prv = dht::partition_range_vector({pr}) /* FIXME: pr is copied */, cmd, opts, timeout, gt = tracing::global_trace_state_ptr(std::move(trace_state)), allow_limit] (replica::database& db) mutable {
            auto trace_state = gt.get();
            tracing::trace(trace_state, "Start querying singular range {}", prv.front());
            return db.query(gs, *cmd, opts, prv, trace_state, timeout, allow_limit).then([trace_state](std::tuple<lw_shared_ptr<query::result>, cache_temperature>&& f_ht) {
                auto&& [f, ht] = f_ht;
...
storage_proxy::query(schema_ptr s,
    ...
    storage_proxy::coordinator_query_options query_options)
{
    return query_result(std::move(s), std::move(cmd), std::move(partition_ranges), cl, allow_limit, std::move(query_options))
    ...
    storage_proxy::coordinator_query_options query_options)
{
    if (slogger.is_enabled(logging::log_level::trace) || qlogger.is_enabled(logging::log_level::trace)) {
        ...
        return do_query(s, cmd, std::move(partition_ranges), cl, allow_limit, std::move(query_options)).then_wrapped([query_id, cmd, s] (future<result<coordinator_query_result>> f) -> result<coordinator_query_result> {
    ...
    return do_query(s, cmd, std::move(partition_ranges), cl, allow_limit, std::move(query_options));
}
    ...
    try {
        ...
        while (true) {
            ...
            if (qr) {
                ...
                auto cqr = co_await query(schema, cmd, std::move(pr), cl, db::allow_per_partition_rate_limit::no, query_options);
...
void storage_proxy::init_messaging_service(shared_ptr<migration_manager> mm) {
    ...
    ser::storage_proxy_rpc_verbs::register_hint_mutation(&_messaging, [this] <typename... Args>(Args&&... args) { return receive_mutation_handler(_hints_write_smp_service_group, std::forward<Args>(args)..., db::allow_per_partition_rate_limit::no); });
...
storage_proxy::receive_mutation_handler(smp_service_group smp_grp, const rpc::client_info& cinfo, rpc::opt_time_point t, frozen_mutation in, inet_address_vector_replica_set forward,
            gms::inet_address reply_to, unsigned shard, storage_proxy::response_id_type response_id, rpc::optional<std::optional<tracing::trace_info>> trace_info, rpc::optional<db::allow_per_partition_rate_limit> allow_limit_opt) {
        tracing::trace_state_ptr trace_state_ptr;
        auto src_addr = netw::messaging_service::get_source(cinfo);
        auto allow_limit = allow_limit_opt.value_or(db::allow_per_partition_rate_limit::no);
        ...
        return handle_write(src_addr, t, schema_version, std::move(in), std::move(forward), reply_to, shard, response_id,
                ...
                /* apply_fn */ [smp_grp, allow_limit] (shared_ptr<storage_proxy>& p, tracing::trace_state_ptr tr_state, schema_ptr s, const frozen_mutation& m,
                        clock_type::time_point timeout) {
                    return p->mutate_locally(std::move(s), m, std::move(tr_state), db::commitlog::force_sync::no, timeout, smp_grp, allow_limit);
                },
                /* forward_fn */ [allow_limit] (shared_ptr<storage_proxy>& p, netw::messaging_service::msg_addr addr, clock_type::time_point timeout, const frozen_mutation& m,
                    ...
                    return ser::storage_proxy_rpc_verbs::send_mutation(&p->_messaging,
                                            addr, timeout, m, {}, reply_to, shard, response_id, std::move(trace_info), allow_limit);
                });
}
...
storage_proxy::handle_read_data(const rpc::client_info& cinfo, rpc::opt_time_point t, query::read_command cmd, ::compat::wrapping_partition_range pr, rpc::optional<query::digest_algorithm> oda, rpc::optional<db::allow_per_partition_rate_limit> allow_limit_opt) {
        tracing::trace_state_ptr trace_state_ptr;
        auto src_addr = netw::messaging_service::get_source(cinfo);
        if (cmd.trace_info) {
        ...
        return do_with(std::move(pr), std::move(sp), std::move(trace_state_ptr), [this, &cinfo, cmd = make_lw_shared<query::read_command>(std::move(cmd)), src_addr = std::move(src_addr), da, t, allow_limit] (::compat::wrapping_partition_range& pr, shared_ptr<storage_proxy>& p, tracing::trace_state_ptr& trace_state_ptr) mutable {
            p->get_stats().replica_data_reads++;
            auto src_ip = src_addr.addr;
            return _mm->get_schema_for_read(cmd->schema_version, std::move(src_addr), p->_messaging).then([cmd, da, &pr, &p, &trace_state_ptr, t, allow_limit] (schema_ptr s) {
                ...
                return p->query_result_local(std::move(s), cmd, std::move(pr2.first), opts, trace_state_ptr, timeout, allow_limit);
                ...
                using final_tuple_type = rpc::tuple<foreign_ptr<lw_shared_ptr<query::result>>, cache_temperature, std::optional<replica::exception_variant>>;
                tracing::trace(trace_state_ptr, "read_data handling is done, sending a response to /{}", src_ip);
                if (!f.failed()) {
                    auto&& [result, temp] = f.get();
                    return make_ready_future<final_tuple_type>(std::move(result), std::move(temp), std::nullopt);
                } else if (features().cluster_supports_typed_errors_in_read_rpc()) {
                    auto ex = replica::encode_replica_exception(f.get_exception());
                    return make_ready_future<final_tuple_type>(foreign_ptr<lw_shared_ptr<query::result>>(), cache_temperature::invalid(), std::make_optional(std::move(ex)));
                } else {
                    return make_exception_future<final_tuple_type>(f.get_exception());
                }
            });
        });
}
...
storage_proxy::handle_read_mutation_data(const rpc::client_info& cinfo, rpc::opt_time_point t, query::read_command cmd, ::compat::wrapping_partition_range pr) {
        ...
        return do_with(std::move(pr),
            ...
            return _mm->get_schema_for_read(cmd->schema_version, std::move(src_addr), p->_messaging).then([cmd, &pr, &p, &trace_state_ptr, &unwrapped, t] (schema_ptr s) mutable {
                ...
                using final_tuple_type = rpc::tuple<foreign_ptr<lw_shared_ptr<reconcilable_result>>, cache_temperature, std::optional<replica::exception_variant>>;
                ...
                if (!f.failed()) {
                    ...
                    auto ex = replica::encode_replica_exception(f.get_exception());
                    return make_ready_future<final_tuple_type>(foreign_ptr<lw_shared_ptr<reconcilable_result>>(), cache_temperature::invalid(), std::make_optional(std::move(ex)));
                } else {
                    return make_exception_future<final_tuple_type>(f.get_exception());
                }
            });
...
storage_proxy::handle_read_digest(const rpc::client_info& cinfo, rpc::opt_time_point t, query::read_command cmd, ::compat::wrapping_partition_range pr, rpc::optional<query::digest_algorithm> oda, rpc::optional<db::allow_per_partition_rate_limit> allow_limit_opt) {
        tracing::trace_state_ptr trace_state_ptr;
        auto src_addr = netw::messaging_service::get_source(cinfo);
        if (cmd.trace_info) {
            trace_state_ptr = tracing::tracing::get_local_tracing_instance().create_session(*cmd.trace_info);
        ...
        return do_with(std::move(pr), get_local_shared_storage_proxy(), std::move(trace_state_ptr), [this, &cinfo, cmd = make_lw_shared<query::read_command>(std::move(cmd)), src_addr = std::move(src_addr), da, t, allow_limit] (::compat::wrapping_partition_range& pr, shared_ptr<storage_proxy>& p, tracing::trace_state_ptr& trace_state_ptr) mutable {
            p->get_stats().replica_digest_reads++;
            auto src_ip = src_addr.addr;
            return _mm->get_schema_for_read(cmd->schema_version, std::move(src_addr), p->_messaging).then([cmd, &pr, &p, &trace_state_ptr, t, da, allow_limit] (schema_ptr s) {
                auto pr2 = ::compat::unwrap(std::move(pr), *s);
                if (pr2.second) {
                    // this function assumes singular queries but doesn't validate
                ...
                auto timeout = t ? *t : db::no_timeout;
                return p->query_result_local_digest(std::move(s), cmd, std::move(pr2.first), trace_state_ptr, timeout, da, allow_limit);
                ...
                using final_tuple_type = rpc::tuple<query::result_digest, api::timestamp_type, cache_temperature, std::optional<replica::exception_variant>>;
                tracing::trace(trace_state_ptr, "read_digest handling is done, sending a response to /{}", src_ip);
                if (!f.failed()) {
                    auto&& [result, timestamp, temp] = f.get();
                    return make_ready_future<final_tuple_type>(std::move(result), timestamp, std::move(temp), std::nullopt);
                } else if (features().cluster_supports_typed_errors_in_read_rpc()) {
                    auto ex = replica::encode_replica_exception(f.get_exception());
                    return make_ready_future<final_tuple_type>(query::result_digest(), api::missing_timestamp, cache_temperature::invalid(), std::make_optional(std::move(ex)));
                } else {
                    return make_exception_future<final_tuple_type>(f.get_exception());
                }
            });
