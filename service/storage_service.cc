...
class topology_coordinator {
    ...
    future<bool> do_work(group0_guard guard) {
        if (_topo_sm._topology.ustate != topology::upgrade_state::done) {
            return do_upgrade_step(std::move(guard));
        }

        return handle_topology_transition(std::move(guard));
    }
    ...
    // Returns true if it successfully moved the upgrade forward.
    // Precondition: the state machine upgrade state is not at upgrade_state::done.
    future<bool> do_upgrade_step(group0_guard);
...
future<bool> topology_coordinator::do_upgrade_step(group0_guard guard) {
    switch (_topo_sm._topology.ustate) {
    case topology::upgrade_state::not_upgraded:
        // The administrator didn't request the upgrade to start.
        // Wait until there is a change in the topology state machine.
        slogger.info("raft topology: upgrade to topology on raft was not requested yet; will wait");
        co_return false;

    case topology::upgrade_state::build_coordinator_state:
        slogger.info("raft topology: building the initial topology state");
    ...
    case topology::upgrade_state::final_global_barrier: {
        ...
        co_await update_topology_state(std::move(guard), {builder.build()}, "upgrade: marking as complete");
...
future<> topology_coordinator::build_coordinator_state(group0_guard guard) {
    ...
    // Execute a global barrier to make sure that all nodes see
    // the build_coordinator_state upgrade_state
    guard = co_await exec_global_command(std::move(guard),
                raft_topology_cmd{raft_topology_cmd::command::barrier},
                {_raft.id()});
    ...
    auto get_sharding_info_for_host_id = [&] (locator::host_id host_id) -> std::pair<size_t, uint8_t> {
        // TODO: It's fragile, need to properly handle the errors
        // Maybe we should wait until we obtain IPs of all nodes?
        const auto ep = tmptr->get_endpoint_for_host_id(host_id);
    ...
    // Build the initial topology state
    topology_mutation_builder builder(guard.write_timestamp());
    builder.set_version(topology::initial_version)
            ...
            .add_enabled_features(std::set<sstring>()); // TODO
    
    std::set<sstring> enabled_features;
    
    // Build per-node state
    const auto eps = _gossiper.get_endpoints();
    for (const auto& ep: eps) {
        ...
        const auto& epmap = eptr->get_application_state_map();
        const auto host_id = utils::UUID(epmap.at(gms::application_state::HOST_ID).value());
        const auto datacenter = epmap.at(gms::application_state::DC).value();
        const auto rack = epmap.at(gms::application_state::RACK).value();
        const auto tokens_v = tmptr->get_tokens(locator::host_id{host_id});
        const std::unordered_set<dht::token> tokens(tokens_v.begin(), tokens_v.end());
        // const auto tokens = epmap.at(gms::application_state::TOKENS).value();
        const auto release_version = epmap.at(gms::application_state::RELEASE_VERSION).value();
        const auto num_tokens = tokens.size();
        const auto shard_count = epmap.at(gms::application_state::SHARD_COUNT).value();
        const auto ignore_msb = epmap.at(gms::application_state::IGNORE_MSB_BITS).value();
        const auto supported_features_s = epmap.at(gms::application_state::SUPPORTED_FEATURES).value();
        ...
        builder.with_node(raft::server_id(host_id))
                ...
                .set("shard_count", std::stoi(shard_count))
                .set("ignore_msb", (int32_t)std::stoi(ignore_msb))
                .set("supported_features", supported_features);
    }

    builder.add_enabled_features(std::move(enabled_features)); // TODO

    // Commit
    builder.set_upgrade_state(topology::upgrade_state::final_global_barrier);
    // TODO: Can the mutation grow too large?
...
future<> topology_coordinator::run() {
    ...
    while (!_as.abort_requested()) {
        ...
        try {
            ...
            bool had_work = co_await do_work(std::move(guard));
            if (!had_work) {
...
future<> storage_service::raft_initialize_discovery_leader(raft::server& raft_server, const join_node_request_params& params) {
    ...
    while (_topology_state_machine._topology.is_empty()) {
        ...
        builder.add_enabled_features(boost::copy_range<std::set<sstring>>(params.supported_features))
                .set_upgrade_state(topology::upgrade_state::done); // Skip upgrade, start right in the topology-on-raft mode
...
future<> storage_service::start_upgrade_to_raft_topology() {
    ...
    if ((co_await _group0->client().get_group0_upgrade_state()).second != group0_upgrade_state::use_post_raft_procedures) {
        throw std::runtime_error("Upgrade to schema-on-raft didn't complete yet. It is a prerequisite for starting "
                "upgrade to raft topology. Refusing to continue.");
    }
...
void storage_service::maybe_enable_raft_topology_after_shadow_round(const std::unordered_set<gms::inet_address>& initial_contact_nodes) {
    if (utils::get_local_injector().is_enabled("force_gossip_based_join")) {
        return;
    }
    bool checked_any = false;
    for (const auto ep : initial_contact_nodes) {
        const auto state = _gossiper.get_endpoint_state_ptr(ep);
        if (state) {
            checked_any = true;
            ...
            if (it != state->get_application_state_map().end() && it->second.value() == "true") {
                slogger.warn("should_use_raft_topology_verbs: node {} supports raft topology", ep);
                _legacy_topology_change_enabled = false;
                _raft_topology_change_enabled = true;
                return;
            }
            slogger.warn("should_use_raft_topology_verbs: node {} does not support raft topology", ep);
        } else {
            slogger.warn("should_use_raft_topology_verbs: node {} is missing state", ep);
        }
    }
    if (!checked_any) {
        slogger.warn("should_use_raft_topology_verbs: there was no data fetched during the shadow round");
    }
}
...
future<> storage_service::join_token_ring(sharded<db::system_distributed_keyspace>& sys_dist_ks,
    ...
    if ((_topology_state_machine._topology.ustate != topology::upgrade_state::not_upgraded || is_first_node())
    ...
    if (is_replacing()) {
        ...
        maybe_enable_raft_topology_after_shadow_round(initial_contact_nodes);
        if (!_raft_topology_change_enabled) {
    ...
    app_states.emplace(gms::application_state::USES_RAFT_TOPOLOGY_OPS, versioned_value::uses_raft_topology_ops(_raft_topology_change_enabled));
...
void storage_service::set_group0(raft_group0& group0, bool experimental_raft_enabled) {
    _group0 = &group0;
    _experimental_raft = experimental_raft_enabled;
}
...
future<> storage_service::decommission() {
    return run_with_api_lock(sstring("decommission"), [] (storage_service& ss) {
        return seastar::async([&ss] {
            ...
            if (ss._raft_topology_change_enabled) {
            ...
            } else if (ss._legacy_topology_change_enabled) {
                ...
                throw std::runtime_error("decommission not allowed at this time - upgrading to raft topology");
            }
    ...
    return run_with_api_lock(sstring("removenode"), [host_id, ignore_nodes_params = std::move(ignore_nodes_params)] (storage_service& ss) mutable {
        return seastar::async([&ss, host_id, ignore_nodes_params = std::move(ignore_nodes_params)] () mutable {
            if (ss._raft_topology_change_enabled) {
                ...
                throw std::runtime_error("removenode not allowed at this time - upgrading to raft topology");
            }
...
future<> storage_service::check_and_repair_cdc_streams() {
    ...
    } else if (_legacy_topology_change_enabled) {
        return _cdc_gens.local().check_and_repair_cdc_streams();
    } else {
        throw std::runtime_error("checkAndRepairCdcStreams not allowed at this time - upgrading to raft topology");
    }
}
...
future<> storage_service::rebuild(sstring source_dc) {
    return run_with_api_lock(sstring("rebuild"), [source_dc] (storage_service& ss) -> future<> {
        if (ss._raft_topology_change_enabled) {
            ...
            throw std::runtime_error("rebuild not allowed at this time - upgrading to raft topology");
        }
    });
}
...
future<join_node_request_result> storage_service::join_node_request_handler(join_node_request_params params) {
    ...
    if (_legacy_topology_change_enabled) {
        result.result = join_node_request_result::rejected{
            .reason = "The cluster did not start upgrading to raft topology yet",
        };
        co_return result;
    }
    ...
    if (!_raft_topology_change_enabled) {
        ...
        while (_group0_as.abort_requested() && !_raft_topology_change_enabled) {
