...
future<> storage_service::sync_raft_topology_nodes(mutable_token_metadata_ptr tmptr, std::optional<locator::host_id> target_node, std::unordered_set<raft::server_id> prev_normal) {
    ...
    auto process_normal_node = [&] (raft::server_id id, const replica_state& rs) -> future<> {
        ...
        if (is_me(host_id)) {
            ...
            co_await set_gossip_tokens(_gossiper, rs.ring.value().tokens, _topology_state_machine._topology.current_cdc_generation_id);
...
future<> storage_service::join_token_ring(sharded<db::system_distributed_keyspace>& sys_dist_ks,
    ...
    if (_raft_experimental_topology) {
        ...
        (void)with_gate(_async_gate, [this, &sys_dist_ks, &gossip_feature_enabler] {
            return track_upgrade_progress_to_topology_coordinator(sys_dist_ks).then([this, gossip_feature_enabler] {
                return gossip_feature_enabler ? _gossiper.unregister_(gossip_feature_enabler) : make_ready_future<>();
            });
...
future<> storage_service::track_upgrade_progress_to_topology_coordinator(sharded<db::system_distributed_keyspace>& sys_dist_ks) {
    ...
    co_await _topology_state_machine.event.when([this] {
        return !legacy_topology_change_enabled() || _abort_source.abort_requested();
    });
    ...
    co_await _topology_state_machine.event.when([this] {
        return raft_topology_change_enabled() || _abort_source.abort_requested();
    });
...
future<> storage_service::join_cluster(sharded<db::system_distributed_keyspace>& sys_dist_ks, sharded<service::storage_proxy>& proxy,
    ...
    if (utils::get_local_injector().is_enabled("force_gossip_based_join") && !_sys_ks.local().bootstrap_complete()) {
    ...
    } else if (!_raft_experimental_topology || _group0->client().in_recovery()) {
        ...
        if (_group0->load_my_id() == g0_info.id) {
            ...
            if (result.use_raft_topology) {
                slogger.info("Booting in raft topology operations mode");
                _topology_change_kind_enabled = topology_change_kind::raft;
            } else {
                slogger.info("Booting in legacy topology operations mode because the cluster still doesn't use raft-based topology operations");
                _topology_change_kind_enabled = topology_change_kind::legacy;
            }
        }
    } else {
