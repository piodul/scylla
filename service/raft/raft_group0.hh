...
// Abstracts away the details of a handshake used to join group 0.
...
public:
    ...
    // Called only if the node is not the discovery leader.
    //
    // - If the function returns true, the node has been added to group 0
    //   successfully.
    // - If the function returns false, the procedure has failed and needs
    //   to be retried.
    // - An exception is interpreted as an irrecoverable error and causes
    //   setup_group0 to fail.
    virtual future<bool> perform_handshake(const group0_info& info) = 0;
};
