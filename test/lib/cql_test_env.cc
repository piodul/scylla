...
public:
    ...
    virtual future<::shared_ptr<cql_transport::messages::result_message>> execute_cql(sstring_view text) override {
        ...
        return local_qp().execute_direct(text, *qs, *qo).then([qs, qo] (auto msg) {
            msg->throw_if_exception();
            return std::move(msg);
        });
    }
        ...
        std::unique_ptr<cql3::query_options> qo) override
    {
        ...
        auto qs = make_query_state();
        auto& lqo = *qo;
        return local_qp().execute_direct(text, *qs, lqo).then([qs, qo = std::move(qo)] (auto msg) {
            msg->throw_if_exception();
            return std::move(msg);
        });
    }
    ...
    future<::shared_ptr<cql_transport::messages::result_message>> execute_batch(
        ...
        auto qs = make_query_state();
        auto& lqo = *qo;
        return local_qp().execute_batch(batch, *qs, lqo, {}).then([qs, batch, qo = std::move(qo)] (auto msg) {
            msg->throw_if_exception();
            return std::move(msg);
        });
    }
