...
async def test_shares_check(manager: ManagerClient):
    ...
    with pytest.raises(InvalidRequest, match="`shares` option can only be used when the cluster is fully upgraded to support workload prioritization"):
...
async def test_workload_prioritization_upgrade(manager: ManagerClient):
    # This test simulates upgrade path for v1 (non-raft) service levels,
    # from a version which does not support workload prioritization
    # to one that does.
