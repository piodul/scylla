...
static void test_pre_post_image(cql_test_env& e, const std::vector<image_persistence_test>& tests) {
    ...
    const bool preimage = true;
    const bool postimage = true;
    ...
    for (const auto& t : tests) {
        ...
        std::map<bytes, std::vector<std::vector<bytes_opt>>> groups;
...
SEASTAR_THREAD_TEST_CASE(test_batch_pre_post_image) {
    do_with_cql_env_thread([] (cql_test_env& e) {
        cquery_nofail(e, "CREATE TABLE ks.tbl (pk int, ck int, s int STATIC, v1 int, v2 int, vm map<int, int>, PRIMARY KEY(pk, ck)) WITH cdc = {'enabled':'true', 'preimage':'true', 'postimage':'true'}"s);
