...
messaging_service::messaging_service(qos::service_level_controller& sl_controller, locator::host_id id, gms::inet_address ip, uint16_t port, gms::feature_service& feature_service, gms::gossip_address_map& address_map)
    : messaging_service(sl_controller, config{std::move(id), ip, ip, port},
...
messaging_service::scheduling_group_for_isolation_cookie(const sstring& isolation_cookie) const {
    ...
    if (tenant_connection(isolation_cookie)) {
        ...
        // When upgrading from an older enterprise version (<= 2024.2) the isolation cookie is not
        ...
        // it is not bullet proof in the sense that if a new tenant class happens to have the exact
    ...
    if (_sl_controller.has_service_level(service_level_name)) {
        ...
        // This also includes `$user` tenant which was used in some open source releases (until and including 6.2).
...
unsigned messaging_service::add_statement_tenant(sstring tenant_name, scheduling_group sg) {
    ...
    // this functions as a way to delete an obsolete tenant with the same name but keeping _clients/_clients_with_host_id
