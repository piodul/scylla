...
static db::rate_limiter::can_proceed account_singular_ranges_to_rate_limit(
    ...
    auto* rate_limit_enforcement_info = std::get_if<db::per_partition_rate_limit::account_and_enforce>(&rate_limit_info);
    if (!rate_limit_enforcement_info || classify_request(dbcfg) != request_class::user) {
    ...
    for (const auto& range : ranges) {
        ...
        if (limiter.account_operation(read_label, token, *table_limit, rate_limit_enforcement_info->get_random_variable_as_double()) == db::rate_limiter::can_proceed::no) {
...
future<> database::do_apply(schema_ptr s, const frozen_mutation& m, tracing::trace_state_ptr tr_state, db::timeout_clock::time_point timeout, db::commitlog::force_sync sync, db::per_partition_rate_limit::info rate_limit_info) {
    ...
    auto* rate_limit_enforcement_info = std::get_if<db::per_partition_rate_limit::account_and_enforce>(&rate_limit_info);
    if (rate_limit_enforcement_info && classify_request(_dbcfg) == request_class::user) {
        if (auto table_limit = s->per_partition_rate_limit_options().get_max_writes_per_second()) {
            ...
            if (_rate_limiter.account_operation(write_label, token, *table_limit, rate_limit_enforcement_info->get_random_variable_as_double()) == db::rate_limiter::can_proceed::no) {
                co_await coroutine::return_exception(replica::rate_limit_exception());
            }
        }
    }
