...
future<> write_hashes(service::storage_proxy& proxy, redis::redis_options& options, bytes&& key, bytes&& field, bytes&& data, long ttl, service_permit permit) {
    ...
    return proxy.mutate(std::vector<mutation> {std::move(m)}, write_consistency_level, timeout, nullptr, std::move(permit).with_allow_limit(db::allow_per_partition_rate_limit::yes));
}
...
future<> delete_fields(service::storage_proxy& proxy, redis::redis_options& options, bytes&& key, std::vector<bytes>&& fields, service_permit permit) {
    ...
    return proxy.mutate(mutations, write_consistency_level, timeout, nullptr, std::move(permit).with_allow_limit(db::allow_per_partition_rate_limit::yes));
}
