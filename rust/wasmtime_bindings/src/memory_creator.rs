...
unsafe impl LinearMemory for ScyllaLinearMemory {
    ...
    fn grow_to(&mut self, new_size: usize) -> Result<()> {
        ...
        if new_size_aligned > max_size {
            return Err(anyhow!("memory grow failed: new size {} exceeds maximum size {}", new_size_aligned, max_size));
        }
        let new_ptr: *mut u8;
...
unsafe impl wasmtime::MemoryCreator for ScyllaMemoryCreator {
    fn new_memory(
        ...
        assert!(!ty.is_64());
        let max_size = std::cmp::min(self.max_scylla_size, maximum.unwrap_or(u32::MAX as usize));
