...
fn create_instance(engine: &Engine, module: &Module, store: &mut Store) -> Result<Box<Instance>> {
    ...
    if let Err(e) = wasmtime_wasi::add_to_linker(&mut linker, |s| s) {
        return Err(anyhow!("Failed to add wasi to linker: {:?}", e));
    }
    let mut inst_fut =
...
fn create_store(engine: &mut Engine, total_fuel: u64, yield_fuel: u64) -> Result<Box<Store>> {
    ...
    store.out_of_fuel_async_yield(((total_fuel + yield_fuel - 1) / yield_fuel) as u64, yield_fuel);
    Ok(Box::new(Store {
...
fn create_engine(max_size: u32) -> Result<Box<Engine>> {
    ...
    // Parallel compilation is enabled by default, but we can't use it in a seastar context.
    config.parallel_compilation(false);
    // ScyllaMemoryCreator uses malloc (from seastar) to allocate linear memory
    config.with_host_memory(std::sync::Arc::new(ScyllaMemoryCreator::new(max_size as usize)));
...
impl Val {
    fn kind(self: &Val) -> Result<ffi::ValKind> {
...
impl<'a> Fut<'a> {
    fn resume(self: &mut Fut<'a>) -> Result<bool> {
        match self
            .fut
            .as_mut()
