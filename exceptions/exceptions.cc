...
rate_limit_exception::rate_limit_exception(const sstring& ks, const sstring& cf, int64_t token) noexcept
    : cassandra_exception(exception_code::CONFIG_ERROR, prepare_message("Rate limit reached for partition with token {} in table {}.{}", token, ks, cf))
    { }
