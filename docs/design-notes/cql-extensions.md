...
Rejected requests receive the scylla-specific `RateLimitExceeded` error.
If the driver doesn't support it, `Config_error` will be sent instead.

For more details, see:

- Detailed [`design notes`](./per-partition-rate-limit.md)
- Description of the [`RateLimitExceeded`](./protocol-extensions.md#rate-limit-error) error
