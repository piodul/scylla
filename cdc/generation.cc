...
static future<> update_streams_description(
        cdc::generation_id gen_id,
        bool raft_experimental_topology,
        shared_ptr<db::system_keyspace> sys_ks,
    ...
    try {
        ...
        })(gen_id, raft_experimental_topology, std::move(sys_ks), std::move(sys_dist_ks), std::move(get_num_token_owners), abort_src));
    }
}
...
future<> generation_service::check_and_repair_cdc_streams() {
    ...
    if (!latest) {
        ...
        try {
            gen = co_await retrieve_generation_data(*latest, _cfg.raft_experimental_topology, *sys_ks, *sys_dist_ks, { tmptr->count_normal_token_owners() });
...
future<> generation_service::legacy_handle_cdc_generation(std::optional<cdc::generation_id> gen_id) {
    ...
    if (using_this_gen) {
        ...
        co_await update_streams_description(*gen_id, _cfg.raft_experimental_topology, get_sys_ks(), get_sys_dist_ks(),
    ...
    (void)(([] (cdc::generation_id gen_id, shared_ptr<generation_service> svc) -> future<> {
        while (true) {
            ...
            try {
                ...
                if (using_this_gen) {
                    cdc_log.info("Starting to use generation {}", gen_id);
                    co_await update_streams_description(gen_id, svc->_cfg.raft_experimental_topology, svc->get_sys_ks(), svc->get_sys_dist_ks(),
...
future<bool> generation_service::legacy_do_handle_cdc_generation(cdc::generation_id gen_id) {
    ...
    auto sys_ks = get_sys_ks();
    auto sys_dist_ks = get_sys_dist_ks();
    auto gen = co_await retrieve_generation_data(gen_id, _cfg.raft_experimental_topology, *sys_ks, *sys_dist_ks, { _token_metadata.get()->count_normal_token_owners() });
    if (!gen) {
...
shared_ptr<db::system_keyspace> generation_service::get_sys_ks() {
    assert_shard_zero(__PRETTY_FUNCTION__);

    if (!_sys_ks.local_is_initialized()) {
        throw std::runtime_error("system keyspace not initialized");
    }

    return _sys_ks.local_shared();
}
