    ...
    auto register_counters = [this] (counters& counters, sstring kind) {
        ...
        _metrics.add_group(cdc_group_name, {
                ...
                sm::make_total_operations("preimage_selects_" + kind, counters.preimage_selects,
                        sm::description(format("number of {} preimage selects", kind)),
                        {})
            });
...
cdc::operation_result_tracker::~operation_result_tracker() {
    auto update_stats = [this] (stats::counters& counters) {
        ...
        counters.touches.apply(_touched_parts);
    };
...
cdc::cdc_service::impl::augment_mutation_call(lowres_clock::time_point timeout, std::vector<mutation>&& mutations, tracing::trace_state_ptr tr_state) {
    ...
    return do_with(std::move(mutations), service::query_state(service::client_state::for_internal_calls(), empty_service_permit()), stats::part_type_set(), false,
            [this, timeout, i, tr_state = std::move(tr_state)] (std::vector<mutation>& mutations, service::query_state& qs,
                    stats::part_type_set& touched_parts, bool& was_split) {
        return transform_mutations(mutations, 1, [this, &mutations, timeout, &qs, tr_state = tr_state, &touched_parts, &was_split] (int idx) mutable {
            ...
            return f.then([trans = std::move(trans), &mutations, idx, tr_state = std::move(tr_state), &touched_parts, &was_split] (lw_shared_ptr<cql3::untyped_result_set> rs) {
                auto& m = mutations[idx];
                auto& s = m.schema();
        ...
        }).then([this, tr_state, &touched_parts, &was_split](std::vector<mutation> mutations) {
            tracing::trace(tr_state, "CDC: Finished generating all log mutations");
            auto tracker = make_lw_shared<cdc::operation_result_tracker>(_ctxt._proxy.get_cdc_stats(), touched_parts, was_split);
