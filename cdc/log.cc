...
public:
            ...
            const mutation& m)
    {
      ...
      try {
        return _ctx._proxy.query(_schema, std::move(command), std::move(partition_ranges), select_cl, db::allow_per_partition_rate_limit::no, service::storage_proxy::coordinator_query_options(default_timeout(), empty_service_permit(), client_state)).then(
