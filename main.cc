...
// the ticket about migrating to libunwind: scylladb/seastar#1878
          ...
          shared_token_metadata::mutate_on_all_shards(token_metadata, [host_id, endpoint = broadcast_addr] (locator::token_metadata& tm) {
            ...
            messaging.start(std::ref(sl_controller), mscfg, scfg, creds, std::ref(feature_service), std::ref(gossip_address_map)).get();
            ...
            auto stop_server_service_levels_api = defer_verbose_shutdown("service levels API", [&ctx] {
                api::unset_server_service_levels(ctx).get();
            });
