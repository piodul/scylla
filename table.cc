...
table::seal_active_streaming_memtable_immediate(flush_permit&& permit) {
  ...
  return with_scheduling_group(_config.streaming_scheduling_group, [this, permit = make_lw_shared(std::move(fp))] () mutable {
    ...
    return with_gate(_streaming_flush_gate, [this, old, permit = std::move(permit)] () mutable {
      return do_with(std::vector<sstables::shared_sstable>(), [this, old, permit = std::move(permit)] (std::vector<sstables::shared_sstable>& newtabs) {
        auto metadata = mutation_source_metadata{};
        metadata.min_timestamp = old->get_min_timestamp();
        metadata.max_timestamp = old->get_max_timestamp();
        auto consumer = _compaction_strategy.make_interposer_consumer(metadata, [this, old, permit, &newtabs] (flat_mutation_reader reader) {
            auto&& priority = service::get_local_streaming_priority();
            sstables::sstable_writer_config cfg = get_sstables_manager().configure_writer("streaming");
        ...
        auto f = consumer(old->make_flush_reader(
                old->schema(),
                compaction_concurrency_semaphore().make_tracking_only_permit(old->schema().get(), "seal_active_streaming_memtable_immediate"),
        ...
        // priority inversion.
        return with_scheduling_group(default_scheduling_group(), [this, old, &newtabs, f = std::move(f)] () mutable {
            return f.then([this, &newtabs, old] {
                ...
                // TODO: Consider this, but I and PiotrJ think that it might not be a good idea
                // return with_scheduling_group(_config.memtable_to_cache_scheduling_group, [this, &newtabs, old] {
            ...
            }).handle_exception([old, &newtabs] (auto ep) {
