...
private:
    ...
    merger_vector<reader_and_fragment> _new_fragments;
...
future<> mutation_reader_merger::prepare_next(db::timeout_clock::time_point timeout) {
    return parallel_for_each(_next, [this, timeout] (reader_and_last_fragment_kind rk) {
        return (*rk.reader)(timeout).then([this, rk] (mutation_fragment_opt mfo) {
            if (mfo) {
                _new_fragments.emplace_back(rk.reader, std::move(*mfo));
...
future<mutation_reader_merger::mutation_fragment_batch> mutation_reader_merger::operator()(db::timeout_clock::time_point timeout) {
    ...
    if (_new_fragments.size() == 1) {
        auto& rk = _new_fragments.front();
        if (rk.fragment.is_partition_start()) {
            _reader_heap.emplace_back(std::move(rk));
            boost::push_heap(_reader_heap, reader_heap_compare(*_schema));
            _gallop_mode_hits = 0;
        } else {
            if (in_gallop_mode()) {
                if (_fragment_heap.empty() || position_in_partition::less_compare(*_schema)(rk.fragment.position(), _fragment_heap.front().fragment.position())) {
                    const auto kind = rk.fragment.mutation_fragment_kind();
                    _current.push_back(std::move(rk.fragment));
                    _next.emplace_back(rk.reader, kind);
                    _new_fragments.clear();
                    return make_ready_future<mutation_fragment_batch>(_current);
                }
                _gallop_mode_hits = 0;
            }

            const auto the_reader = rk.reader;
            _fragment_heap.emplace_back(std::move(rk));
            boost::range::push_heap(_fragment_heap, fragment_heap_compare(*_schema));

            // Increase gallop counter if it was the best reader
            if (_reader_heap.front().reader == the_reader) {
                ++_gallop_mode_hits;
            } else {
                _gallop_mode_hits = 1;
            }
