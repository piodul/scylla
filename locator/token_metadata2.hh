...
class token_metadata2;
class tablet_metadata;

struct host_id_or_endpoint2 {
    host_id id;
    ...
    host_id_or_endpoint2(const sstring& s, param_type restrict = param_type::auto_detect);
    ...
    // using the token_metadata2
    void resolve(const token_metadata2& tm);
};

class token_metadata_impl;
struct topology_change_info2;

class token_metadata2 final {
...
private:
    ...
    public:
        ...
        tokens_iterator(const token& start, const token_metadata_impl* token_metadata2);
...
public:
    token_metadata2(config cfg);
    explicit token_metadata2(std::unique_ptr<token_metadata_impl> impl);
    token_metadata2(token_metadata2&&) noexcept; // Can't use "= default;" - hits some static_assert in unique_ptr
    token_metadata2& operator=(token_metadata2&&) noexcept;
    ~token_metadata2();
    ...
    // It must be called only on a temporary copy of the token_metadata2
    ...
    host_id_or_endpoint2 parse_host_id_and_endpoint(const sstring& host_id_string) const;
    ...
    /**
     * Create a full copy of token_metadata2 using asynchronous continuations.
     * The caller must ensure that the cloned object will not change if
     * the function yields.
     */
    future<token_metadata2> clone_async() const noexcept;
     ...
     * the function yields.
     */
    future<token_metadata2> clone_only_token_map() const noexcept;
    /**
     ...
     */
    future<token_metadata2> clone_after_all_left() const noexcept;

    /**
     * Gently clear the token_metadata2 members.
...
struct topology_change_info2 {
    token_metadata2 target_token_metadata;
    std::optional<token_metadata2> base_token_metadata;
    std::vector<dht::token> all_tokens;
    token_metadata2::read_new_t read_new;

    topology_change_info2(token_metadata2 target_token_metadata_,
...
using token_metadata_ptr2 = lw_shared_ptr<const token_metadata2>;
using mutable_token_metadata_ptr2 = lw_shared_ptr<token_metadata2>;
...
mutable_token_metadata_ptr2 make_token_metadata_ptr2(Args... args) {
    return make_lw_shared<token_metadata2>(std::forward<Args>(args)...);
}

// class shared_token_metadata {
//     mutable_token_metadata_ptr2 _shared;
//     token_metadata_lock_func _lock_func;

//     // We use this barrier during the transition to a new token_metadata2 version to ensure that the
//     // system stops using previous versions. Here are the key points:
//     //   * A new phase begins when a mutable_token_metadata_ptr2 passed to shared_token_metadata::set has
...
//     //   * The operation is stored within the new token_metadata2 instance (token_metadata2::set_version_tracker),
...
//     //   transition to complete. Once this future resolves, there will be no token_metadata2 instances
...
//     token_metadata_ptr2 get() const noexcept {
//         return _shared;
//     }

//     void set(mutable_token_metadata_ptr2 tmptr) noexcept;
...
//     // clones the token_metadata2 (using clone_async)
//     // and calls an asynchronous functor on
//     // the cloned copy of the token_metadata2 to mutate it.
//     //
...
// std::unique_ptr<locator::token_range_splitter> make_splitter(token_metadata_ptr2);
