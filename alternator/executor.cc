        ...
        alternator::stats& stats)
{
    ...
    return proxy.query(schema, command, to_partition_ranges(*schema, pk), cl, db::allow_per_partition_rate_limit::yes, service::storage_proxy::coordinator_query_options(executor::default_timeout(), std::move(permit), client_state)).then(
...
future<executor::request_return_type> executor::get_item(client_state& client_state, tracing::trace_state_ptr trace_state, service_permit permit, rjson::value request) {
    ...
    return _proxy.query(schema, std::move(command), std::move(partition_ranges), cl, db::allow_per_partition_rate_limit::no,
...
future<executor::request_return_type> executor::batch_get_item(client_state& client_state, tracing::trace_state_ptr trace_state, service_permit permit, rjson::value request) {
    ...
    for (const auto& rs : requests) {
        for (const auto &r : rs.requests) {
            ...
            future<std::tuple<std::string, std::optional<rjson::value>>> f = _proxy.query(rs.schema, std::move(command), std::move(partition_ranges), rs.cl, db::allow_per_partition_rate_limit::yes,
